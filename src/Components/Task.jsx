...
export const Task = ({ task }) => {
    ...
    const completeMarker = task.completed ? "line-through" : "";
    ...
    return (
        <li key={task.id} className="w-full flex flex-col gap-4 p-5 shadow-md">
            ...
            <div className="flex flex-col w-full gap-1 ">
                <h2 className={"text-black text-xl font-medium capitalize break-words " + completeMarker}>{task.title} </h2>
                <p className={"text-gray-600 black text-md capitalize break-words " + completeMarker}>{task.description} </p>
            </div>
